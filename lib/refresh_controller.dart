...
class RefreshController extends RiveAnimationController<RuntimeArtboard> {
  ...
  void apply(RuntimeArtboard artboard, double elapsedSeconds) {
    ...
    if (refreshState == RefreshIndicatorMode.refresh ||
      ...
      // Loading animation
      if (_trigger.time >=
          _trigger.animation.workEnd / _trigger.animation.fps) {
  ...
  void reset() {
    if (pulledExtent != null && triggerThreshold != null) {
      if (pulledExtent < triggerThreshold) {
        final triggerStartFrame = _trigger.animation.enableWorkArea
            ? _trigger.animation.workStart
            : 0;
        _trigger.time = triggerStartFrame.toDouble() / _trigger.animation.fps;

        final loadingStartFrame = _loading.animation.enableWorkArea
            ? _loading.animation.workStart
            : 0;
